...
A fundamental property: $D_{KL}(P \| Q) \geq 0$ with equality iff $P = Q$ almost everywhere. The proof follows directly from Jensen's inequality applied to the convex function $-\log$:

$$D_{KL}(P \| Q) = -\mathbb{E}_P\left[\log \frac{q(X)}{p(X)}\right] \geq -\log \mathbb{E}_P\left[\frac{q(X)}{p(X)}\right] = -\log \sum_x p(x) \frac{q(x)}{p(x)} = -\log 1 = 0$$
...
1. **Asymmetry:** $D_{KL}(P \| Q) \neq D_{KL}(Q \| P)$ in general. The direction matters enormously.
2. **No triangle inequality:** There exist $P, Q, R$ where $D_{KL}(P \| R) > D_{KL}(P \| Q) + D_{KL}(Q \| R)$.

This asymmetry is not a defect — it reflects genuinely different optimization problems, as we'll see next.

> **Key insight:** KL divergence is not a distance. It is a *directed* measure: $D_{KL}(P \| Q)$ asks "how expensive is it to use $Q$ when the truth is $P$?" Swapping the arguments changes the question entirely.
...
Minimizing $D_{KL}(P \| Q)$ with respect to $Q$ expands the expectation:
...
Minimizing $D_{KL}(Q \| P)$ with respect to $Q$:
...
This distinction is critical in variational inference: the standard ELBO objective optimizes the reverse KL $D_{KL}(q(z|x) \| p(z|x))$, which is why variational posteriors tend to be mode-seeking and underestimate uncertainty.
...
:::quiz
question: "You are fitting a unimodal Gaussian $q$ to a bimodal distribution $p$ with modes at $x=-3$ and $x=3$. Under forward KL $D_{KL}(p \\| q)$, where will $q$ center?"
options:
  - "At $x = -3$"
  - "At $x = 3$"
...
where $f^*$ is the convex conjugate of $f$. This unifies many GAN variants: the original GAN approximately minimizes Jensen-Shannon divergence (an f-divergence), while other choices of $f$ yield different training dynamics.
...
$$D_{KL}(P \| Q) = \sup_{T: \mathbb{E}_Q[e^T] < \infty} \left[\mathbb{E}_P[T] - \log \mathbb{E}_Q[e^T]\right]$$
...
$$\log p_\theta(x) = \underbrace{\mathbb{E}_{q_\phi(z|x)}[\log p_\theta(x|z)] - D_{KL}(q_\phi(z|x) \| p(z))}_{\text{ELBO}(x; \theta, \phi)} + D_{KL}(q_\phi(z|x) \| p_\theta(z|x))$$
...
2. **Minimizes rate** — the $D_{KL}(q_\phi(z|x) \| p(z))$ term regularizes the encoder toward the prior.
...
:::quiz
question: "In the VAE ELBO, what happens if $D_{KL}(q(z|x) \\| p(z)) = 0$ for all $x$?"
options:
...
correct: 1
explanation: "If $D_{KL}(q(z|x) \\| p(z)) = 0$, then $q(z|x) = p(z)$ for all $x$ — the encoder's output doesn't depend on $x$. This is posterior collapse: the latent code carries zero information about the input, and the decoder must generate outputs from the prior alone."
:::
...
:::quiz
question: "The Donsker-Varadhan representation $D_{KL}(P \\| Q) = \\sup_T [\\mathbb{E}_P[T] - \\log \\mathbb{E}_Q[e^T]]$ is useful because:"
options:
