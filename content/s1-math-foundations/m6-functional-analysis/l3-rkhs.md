...
> **Remember:** Bochner's theorem in its general form states that a continuous function $k(x,y) = k(x-y)$ is positive definite if and only if it equals the Fourier transform of a positive finite Borel measure $\Lambda$ (not necessarily a probability measure): $k(x-y) = \int e^{i\omega^\top(x-y)} d\Lambda(\omega)$. Normalizing to $k(0) = 1$ makes $\Lambda$ a probability measure and gives the sampling-based form $k(x,y) = \mathbb{E}_{\omega \sim \Lambda}[e^{i\omega^\top(x-y)}]$. The RFF approximation samples $\omega_1,\ldots,\omega_D \sim \Lambda$ and uses the real part: $z(x) = \sqrt{2/D}[\cos(\omega_1^\top x + b_1), \ldots, \cos(\omega_D^\top x + b_D)]$ with $b_d \sim \text{Uniform}[0,2\pi]$, giving $\hat{k}(x,y) = z(x)^\top z(y) \approx k(x,y)$ (the imaginary parts cancel for symmetric real-valued kernels).
...
**RKHS as the Cameron-Martin space.** The RKHS $H_k$ is the Cameron-Martin space of the GP: it consists of the "directions" in function space along which the GP measure can be shifted while remaining absolutely continuous with respect to itself. Functions in $H_k$ have finite log-likelihood ratio under the GP; functions outside $H_k$ are in the support of the GP measure but have infinite regularization cost. A fundamental result (Driscoll, 1973) establishes the opposite: for a non-degenerate Gaussian process with covariance kernel $k$, almost all sample paths lie **outside** $\mathcal{H}_k$. The Cameron-Martin space $\mathcal{H}_k$ is the space of deterministic mean functions that can shift the GP measure (in the sense of absolute continuity of measures); actual sample paths live in a larger support set. The analogy with Brownian motion is instructive: its Cameron-Martin space is $H^1_0([0,1])$ (absolutely continuous paths with square-integrable derivative), but Brownian sample paths are a.s. only Hölder-$1/2$ and hence a.s. outside $H^1_0$. For ML: this explains why GP posterior means (which lie in $\mathcal{H}_k$) are smoother than typical sample paths — mean functions are always in the RKHS, sample functions almost never are.
